...
class wppbot:
    ...
    def send_msg(self,site,template_response):
        ...
        try:
            ...
            if buttton_invalida_wpp.text == Tag.BOTAO_DE_CONFIRMACAO_OK_DO_ELEMENTO_INVALIDO.value:
                ...
                print("Clicou no inválido")
        except:
            pass

        self.queda_de_conexao_tentar_novamente()
    ...
    def queda_de_conexao_tentar_novamente(self):
        tentar_novamente = True
        while True:
            if tentar_novamente is True:
                try:
                    time.sleep(30)
                    button_tentar_novamente = self.driver.find_element_by_xpath(Tag.BOTAO_TENTAR_NOVAMENTE.value)
                    if button_tentar_novamente.text == Tag.TEXTO_TENTAR_NOVAMENTE.value:
                        button_tentar_novamente.click()
